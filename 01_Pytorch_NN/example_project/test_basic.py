...
def test_arange_elems():
    ...
    assert torch.allclose(arr[-1], torch.tensor(9.0, dtype=torch.float))
...
def test_div_zero():
    ...
    result = b/a
    assert torch.isinf(result).item() 
...
def test_accuracy_parametrized(preds, targets, result):
    assert torch.allclose(compute_accuracy(preds, targets), torch.tensor(result, dtype=torch.float), rtol=1e-2, atol=1e-2)
